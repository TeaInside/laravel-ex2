...
class DepositController extends BaseController
{
    public function generateNewAddrDeposit()
    {
        ...
        if (isset($addr_deposit->addr_deposit) && $deposit->addressIsDesposited($addr_deposit->addr_deposit)) {
            ...
            echo json_encode(array('status'=>'error','message'=>'You cannot generate a new address, without using the old address'));
            exit;
        }
    }
    ...
    public function cronUpdateDeposit()
    {
            ...
            try {
                ...
                foreach ($listtrans as $key => $value) {
                    ...
                    if ($trans != null) {
                        ...
                        //Log::info( "\n"."transaction: ",$trans);
                        //mail("anoxydoxy@gmail.com", 'Deposit Cron: ', var_export($trans, true));
                        ...
                        if (isset($deposit->id)) {
                            ...
                            if ($deposit->paid == 0) {
                                if ($category == "receive" && $confirms > $wallet->confirm_count && isset($user->id)) {
                                    Deposit::where('transaction_id', $deposit->transaction_id)->update(array('paid' => 2,'confirmations'=>$confirms));
                            ...
                            if ($category == "receive" && isset($user->id)) {
                                if ($confirms > $wallet->confirm_count) {
        ...
        return view('cronupdatedeposit', $data);
    }
                        ...
                        if (isset($deposit->transaction_id)) {
                            ...
                            if ($category == "receive" && isset($user->id)) {
                                if ($confirms >= $wallet->confirm_count) {
                                    ...
                                    //Log::info( "\n"."This Deposit is unconfirmed. Current confirmations:" . $confirms .". Required : ".$wallet->confirm_count);
                                }
                            } else {
                                //Log::info( "\n"."transaction is not a deposit or account is invalid.");
                            }
