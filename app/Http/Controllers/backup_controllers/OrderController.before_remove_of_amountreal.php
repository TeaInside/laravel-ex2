...
class OrderController extends Controller
{
    ...
    {
        if (Auth::guest()) {
            echo json_encode(array('status'=>'error','message'=> Lang::get('messages.login_to_buy'), 'messages'=>array(Lang::get('messages.login_to_buy')) ));
            exit;
        }
        ...
        $history_trade = array();           //Socket - Visible for all users, implemented in message_socket
        $trade_live = array();          //Socket - Visible for all users, implemented in message_socket
        ...
        if ($balance->takeMoney($total_buy, $wallet_to, $user->id)) {//tru tien mua chua tru phi (giam tien)
            ...
            if (count($sell_orders_matching)>0) {
                    ...
                    if ($amount_buy>0) {
                        ...
                        if ($amount_buy == $amount_sell) {
                            ...
                            //if($balance->takeMoney($fee_buy,$wallet_to,$user->id)){//tru phi nguoi mua
                                ...
                                $message1 .='<p>'.Lang::get('messages.partly_filled').' <br />Buy '.$amount_sell .' '.$from.' @  '.sprintf('%.8f', $price_sell).' Fee buy: '.$fee_buy.' '.$from.'. Received: '.($amount_sell-$fee_buy).' '.$from. ' - Total amount paid: : '.($total_sell).' '.$to.'</p>';
                                //Log::info($message1);
                                $message = array('message' => $message1, 'status' => 'success');
                                ...
                                //call socket
                                //$message_socket['message_socket'][$class_price]['order_s'] = array("action"=>"delete","id"=>$sell_matching['id'], 'price'=>$price_sell);
                                $message_socket['message_socket'][$class_price]['order_s'] = array('action'=>"update",'id'=>$sell_matching['id'], 'price'=>$price_sell, 'a_buy' => $amount_buy, 'a_sell' => $amount_sell, 'a_type' => 'buy',  'amount_real_trading' => $amount_real_trading, 'amount_real_trading_total' => $amount_real_trading_total);
                ...
                // Correct - Sum all the amounts from the specific order, from history trade and add it to the message socket,
                $amount_i = 0;
                $price_i = 0;
                $class_price = '';
                foreach ($history_trade as $ht) {
                    if ($ht['price'] != $price_i) {
                        ...
                        $class_price = str_replace(",", "-", $ht['price']);
                        $class_price = $ht['price'];
                        
                        $trade_live[$class_price]['order_s']['amount']+=$amount_i;
        ...
        if ($balance->takeMoney($amount_sell, $wallet_from, $user->id)) {
            ...
            if (count($buy_orders_matching)>0) {
                    ...
                    if ($amount_sell>0) {
                        ...
                        if ($amount_buy == $amount_sell) {
                            ...
                            //if($balance->takeMoney($fee_buy,$wallet_to,$user->id)){//tru phi nguoi mua
                                ...
                                $message1 .= '<p>'.Lang::get('messages.partly_filled').' <br />Sell '.$amount_buy .' '.$from.' @ '.$price_buy.' Fee sell: '.$fee_sell.' '.$to. ' - Received: '.($total_sell-$fee_sell).' '.$to.'</p>';
                                $message = array('message' => $message1, 'status' => 'success');
                ...
                if ($amount_sell>0) {
                    ...
                    $message_socket_user['user_orders'][$orders_buy->id]['order_b'] = array('action'=>"insert","id"=>$orders_sell->id,"amount"=>$amount_sell,"price"=>$price_sell,"total"=>$total_rest,'type'=>'buy','created_at'=>$orders_sell->created_at);
