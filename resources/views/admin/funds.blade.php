...
<table class="table table-bordered noboder-td">
    <tbody>
       	<tr>
            <td> <h3 style="margin-bottom: 2px;">{{$wallet->type}} - {{$wallet->name}} - {{$wallet->wallet_id}}</h3> <p>Balance: <b>{{$balance}}</b> {{$wallet->type}}</p></td>
            <td>                
...
<div id="form-withdraw">
	<form class="form-horizontal" role="form" autocomplete=off id="form-send-coin" method="POST" action="{{{ Auth::check('admin\\AdminSettingController@doSendCoin') ?: URL::to('/admin/send-coin') }}}">	
		...
		<div class="form-group">
		    ...
		    <div class="col-sm-10">	       
				  ...
				  <input style="display:none;" type="text" class="form-control" name="receive_amount" id="receive_amount" readonly>	
				  <span class="add-on coin_code"></span>
				</div>     
		    </div>	    
		</div> 	
...
(function($){ 
	...
	$("#form-send-coin").validate({
            ...
            messages: {
                address: "Please enter a receive address.",               
                amount: {
